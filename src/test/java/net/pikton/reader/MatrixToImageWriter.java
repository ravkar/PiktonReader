...
public final class MatrixToImageWriter {
  ...
  public static void writeToFile(ByteMatrix matrix, String format, File file, int scale) throws IOException {
	  BufferedImage origImage = toBufferedImage(matrix);    
	  
	  BufferedImage image = new BufferedImage(origImage.getWidth()*scale, origImage.getHeight()*scale, BufferedImage.TYPE_BYTE_BINARY);
		Graphics g = image.getGraphics();
		for (int i = 0; i < origImage.getWidth(); i++){
			for (int j = 0; j < origImage.getHeight(); j++){
				int value = origImage.getRGB(i, j);
				g.setColor(new Color(value));
				g.fillRect(i*scale, j*scale, scale, scale);//(x, y, width, height);								
			}
		}		
	    ImageIO.write(image, format, file);
  }  
